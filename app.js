...
import { Plus, X, Archive, ChevronLeft, ChevronDown, ChevronUp, Download, Upload } from 'lucide-react';
...
export default function BudgetApp() {
  ...
  const loadData = async () => {
    try {
      ...
      const incomesResult = await window.storage.get('budget:incomes');
      const expensesResult = await window.storage.get('budget:expenses');
      const transactionsResult = await window.storage.get(`budget:transactions:${currentPeriod}`);
      const archivedResult = await window.storage.get('budget:archived');
      
      if (incomesResult) setIncomes(JSON.parse(incomesResult.value));
      if (expensesResult) setExpenses(JSON.parse(expensesResult.value));
      if (transactionsResult) setTransactions(JSON.parse(transactionsResult.value));
      if (archivedResult) setArchivedMonths(JSON.parse(archivedResult.value));
    } catch (error) {
      console.log('No saved data found');
    }
  };

  const checkAndArchiveIfNeeded = async () => {
    try {
      const lastCheckResult = await window.storage.get('budget:last_check');
      const currentPeriod = getCurrentPeriod();
      
      if (lastCheckResult && lastCheckResult.value !== currentPeriod) {
        await archiveCurrentMonth(lastCheckResult.value);
      }
      
      await window.storage.set('budget:last_check', currentPeriod);
    } catch (error) {
      const currentPeriod = getCurrentPeriod();
      await window.storage.set('budget:last_check', currentPeriod);
    }
  ...
  const archiveCurrentMonth = async (period) => {
    try {
      const transactionsResult = await window.storage.get(`budget:transactions:${period}`);
      const expensesResult = await window.storage.get('budget:expenses');
      const incomesResult = await window.storage.get('budget:incomes');
      
      if (transactionsResult || expensesResult || incomesResult) {
        const archived = {
          period,
          incomes: incomesResult ? JSON.parse(incomesResult.value) : [],
          expenses: expensesResult ? JSON.parse(expensesResult.value) : [],
          transactions: transactionsResult ? JSON.parse(transactionsResult.value) : [],
          archivedDate: new Date().toISOString()
        };
        
        const archivedResult = await window.storage.get('budget:archived');
        const existing = archivedResult ? JSON.parse(archivedResult.value) : [];
        const updated = [archived, ...existing];
        
        await window.storage.set('budget:archived', JSON.stringify(updated));
        setArchivedMonths(updated);
        
        await window.storage.delete(`budget:transactions:${period}`);
      }
    } catch (error) {
      console.log('Error archiving month:', error);
    }
  };

  const addIncome = async () => {
    if (newIncome.description && newIncome.amount) {
      const updated = [...incomes, { id: Date.now(), ...newIncome }];
      setIncomes(updated);
      await window.storage.set('budget:incomes', JSON.stringify(updated));
  ...
  const addExpense = async () => {
    if (newExpense.name && newExpense.amount) {
      ...
      setExpenses(updated);
      await window.storage.set('budget:expenses', JSON.stringify(updated));
  ...
  const addTransaction = async () => {
    ...
    if (isValid) {
      ...
      await window.storage.set(`budget:transactions:${currentPeriod}`, JSON.stringify(updated));
  ...
  const importData = async (event) => {
    ...
    try {
      ...
      if (data.incomes) {
        ...
        await window.storage.set('budget:incomes', JSON.stringify(data.incomes));
      }
      
      if (data.expenses) {
        setExpenses(data.expenses);
        await window.storage.set('budget:expenses', JSON.stringify(data.expenses));
      }
      
      if (data.transactions) {
        setTransactions(data.transactions);
        await window.storage.set(`budget:transactions:${getCurrentPeriod()}`, JSON.stringify(data.transactions));
      }
      
      if (data.archivedMonths) {
        setArchivedMonths(data.archivedMonths);
        await window.storage.set('budget:archived', JSON.stringify(data.archivedMonths));
      }
          ...
          <div className="flex justify-between items-center mb-4">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="flex items-center gap-1 text-sm text-gray-600 hover:text-gray-800 p-2"
                title="Import Data"
              >
                <Upload size={18} />
              </button>
        ...
        <div className="bg-white rounded-2xl shadow-lg p-6">
          ...
          >
            <div className="flex items-center gap-2">
              ...
              {incomes.length > 0 && (
                <span className="text-sm text-gray-600">(€{totalIncome.toFixed(2)})</span>
              )}
            </div>
            <div className="flex items-center gap-2">
              <button
              ...
              {incomes.length > 0 && (
                incomesExpanded ? <ChevronUp size={20} /> : <ChevronDown size={20} />
              )}
            </div>
          </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm">
            ...
            <div className="flex gap-2">
              <button onClick={() => setShowIncomeModal(false)} className="flex-1 px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50">
                Cancel
              </button>
              <button onClick={addIncome} className="flex-1 px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700">
                Add
              </button>
            </div>
          </div>
      ...
      {showTransactionModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm">
            ...
            <input
              type="text"
              placeholder={newTransaction.type === 'groceries' ? 'Description (optional)' : 'Description (required)'}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm">
            ...
            <p className="text-sm text-gray-600 mb-4">
              Select a backup file to restore your budget data. This will replace all current data.
            </p>
            <input
              type="file"
              accept=".json"
